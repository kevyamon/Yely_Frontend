    ...
    <Box sx={{ width: 280, height: '100%', bgcolor: 'background.paper', color: 'text.primary', p: 2, display: 'flex', flexDirection: 'column' }}>
      ...
      <Box onClick={() => handleNavigate('/profile')} sx={{ mb: 4, mt: 2, display: 'flex', alignItems: 'center', p: 1, cursor: 'pointer', borderRadius: 2, '&:hover': { bgcolor: 'action.hover' } }}>
        ...
        <Box>
          ...
          <Typography variant="caption" color="text.secondary">
            {user?.role === 'superAdmin' ? '👑 SuperAdmin' : user?.role === 'admin' ? '🛡️ Admin' : user?.role === 'driver' ? 'Chauffeur' : 'Passager'}
          </Typography>
        </Box>
      ...
      <List>
        ...
        {(user?.role === 'superAdmin' || user?.role === 'admin') && (
          <ListItemButton onClick={() => handleNavigate('/admin/dashboard')} sx={{ borderRadius: 2, mb: 1, bgcolor: 'rgba(255, 215, 0, 0.1)' }}>
            <ListItemIcon sx={{ color: '#FFD700' }}><DashboardIcon /></ListItemIcon>
            <ListItemText 
